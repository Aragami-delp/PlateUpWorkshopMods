...
namespace KitchenPrepUiDuringDay
{
    ...
    public class PrepUiDuringDayMain : GenericSystemBase, IModSystem, IModInitializer
    {
        ...
        public static bool ShowUiDuringDay;

        public static void ShowPrepUi()
        {
            if (GameInfo.CurrentScene == SceneType.Kitchen)
            {
                if (GameInfo.IsPreparationTime)
                {
                    ParametersDisplayViewGO.SetActive(true);
                }
                else
                {
                    ParametersDisplayViewGO.SetActive(ShowUiDuringDay);
                }
            }
        }
        ...
        protected override void Initialise()
        {
            ...
            if (GameObject.FindObjectOfType<PrepUiDuringDayMono>() == null)
            {
                GameObject thingy = new GameObject("PrepUiDuringDayMono");
                thingy.AddComponent<PrepUiDuringDayMono>();
                GameObject.DontDestroyOnLoad(thingy);
            }
        }
